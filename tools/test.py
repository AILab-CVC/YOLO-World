...
def parse_args():
    parser = argparse.ArgumentParser(description="MMYOLO test (and eval) a model")
    parser.add_argument("config", help="test config file path")
    parser.add_argument("checkpoint", help="checkpoint file")
    parser.add_argument("--work-dir", help="the directory to save the file containing evaluation metrics")
    parser.add_argument("--out", type=str, help="output result file (must be a .pkl file) in pickle format")
    parser.add_argument(
        "--json-prefix",
        type=str,
    ...
    parser.add_argument(
        "--show-dir", help="directory where painted images will be saved. " "If specified, it will be automatically saved " "to the work_dir/timestamp/show_dir"
    )
...
def main():
    ...
    if args.json_prefix is not None:
        cfg_json = {"test_evaluator.format_only": True, "test_evaluator.outfile_prefix": args.json_prefix}
    ...
    if args.tta:
        assert "tta_model" in cfg, "Cannot find ``tta_model`` in config." " Can't use tta !"
        assert "tta_pipeline" in cfg, "Cannot find ``tta_pipeline`` " "in config. Can't use tta !"
