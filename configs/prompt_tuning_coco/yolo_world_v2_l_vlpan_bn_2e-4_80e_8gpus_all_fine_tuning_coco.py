...
model = dict(
    ...
    backbone=dict(_delete_=True, type="MultiModalYOLOBackbone", text_model=None, image_model={{_base_.model.backbone}}, with_text_model=False),
    neck=dict(
        type="YOLOWorldPAFPN",
        freeze_all=False,
    ...
    bbox_head=dict(
        type="YOLOWorldHead",
        head_module=dict(type="YOLOWorldHeadModule", freeze_all=False, use_bn_head=True, embed_dims=text_channels, num_classes=num_training_classes),
    ),
    train_cfg=dict(assigner=dict(num_classes=num_training_classes)),
)

# dataset settings
final_transform = [dict(type="mmdet.PackDetInputs", meta_keys=("img_id", "img_path", "ori_shape", "img_shape", "flip", "flip_direction"))]
mosaic_affine_transform = [
    dict(type="Mosaic", img_scale=_base_.img_scale, pad_val=114.0, pre_transform=_base_.pre_transform),
    dict(
...
train_pipeline = [
    ...
    *mosaic_affine_transform,
    dict(type="YOLOv5MixUp", prob=_base_.mixup_prob, pre_transform=[*_base_.pre_transform, *mosaic_affine_transform]),
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        bias_decay_mult=0.0,
        norm_decay_mult=0.0,
        custom_keys={"backbone.text_model": dict(lr_mult=0.01), "logit_scale": dict(weight_decay=0.0), "embeddings": dict(weight_decay=0.0)},
    ),
    constructor="YOLOWv5OptimizerConstructor",
)

# evaluation settings
val_evaluator = dict(_delete_=True, type="mmdet.CocoMetric", proposal_nums=(100, 1, 10), ann_file="data/coco/annotations/instances_val2017.json", metric="bbox")
