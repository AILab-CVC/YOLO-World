...
**Date:** 2025.02.05\
**Note:** Yixiao Ge is the corresponding contributor.
...
Subsequently, the image embeddings corresponding to different category query boxes will replace text embeddings in forward computation. Objects are matched to their respective query boxes based on categories (where one of the ground truth bboxes of the same category is sampled as a query bbox). Then, the loss is calculated to optimize the adapter's parameters.


<table>
    <tr>
    <th>Model</th><th>Image Prompts</th><th>Training</th><th>COCO AP</th>
    </tr>
    <tr>
    <td>YOLO-World-L</td>
    </tr>
