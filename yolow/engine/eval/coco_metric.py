...
from ...dist_utils import _to_cpu, broadcast_object_list, collect_results, is_main_process
...
class CocoMetric:
    ...
    def __init__(self,
        ...
        if self.prefix is None:
            self.logger.warning('The prefix is not set in metric class '
        ...
        if ann_file is not None:
            ...
            self._coco_api = COCO(osp.join(osp.abspath(os.getcwd()), ann_file))
            if sort_categories:
    ...
    def gt_to_coco_json(self, gt_dicts: Sequence[dict], outfile_prefix: str) -> str:
        ...
        info = dict(
            date_created=str(datetime.datetime.now()), description='Coco json file converted by mmdet CocoMetric.')
        coco_json = dict(
            info=info,
            images=image_infos,
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for metric in self.metrics:
            ...
            try:
                ...
                self.logger.error('The testing results of the whole dataset is empty.')
                break
            ...
            if metric == 'proposal':
                ...
                self.logger.info(f'{metric}_mAP_copypaste: {ap[0]:.3f} '
    ...
    def evaluate(self, size: int) -> dict:
        ...
        if len(self.results) == 0:
            self.logger.warning(f'{self.__class__.__name__} got empty `self.results`. Please '
